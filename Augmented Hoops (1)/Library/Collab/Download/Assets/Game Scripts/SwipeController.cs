        ...
        if (!hasShot && rigidBody.name != "basketball" && Input.touchCount > 0)
        {
            ...
            if (Input.GetTouch(0).phase == TouchPhase.Moved)
            {
                ...
                Vector3 position = arCamera.transform.position +
                                       (arCamera.transform.right * (xPos / (Screen.width / xMul))) +
                                       (arCamera.transform.up * (yPos / (Screen.height / yMul))) +
                                       (arCamera.transform.forward * 4.0f);
